...
function GetWindowText(Winhandle: LongInt): String;
function GetClassname(WinHandle: LongInt): String;

implementation

function GetWindowText(Winhandle: LongInt): String;
var
	Cap: array[1..64] of Char;
begin
	Windows.GetWindowText(Winhandle, @Cap, SizeOf(Cap));
  {$IFDEF VER220}
  Result := Cap;
  {$ELSE}
  Result := (StrPas(@Cap));
...
var
	Cap: array[1..64] of Char;
begin
	Windows.GetClassName(Winhandle, @Cap, SizeOf(Cap));
  {$IFDEF VER220}
  Result := Cap;
  {$ELSE}
  Result := (StrPas(@Cap));
